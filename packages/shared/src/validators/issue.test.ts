...
describe("issue validators", () => {
  ...
  it("accepts structured issue comment presentation and metadata", () => {
    const parsed = addIssueCommentSchema.parse({
      ...
      authorType: "system",
      presentation: { kind: "system_notice", tone: "warning", title: "Needs disposition" },
      metadata: { version: 1, sourceRunId: "11111111-1111-4111-8111-111111111111", sections: [{ title: "Evidence", rows: [{ type: "key_value", label: "Cause", value: "successful_run_missing_state" }, { type: "issue_link", label: "Source issue", identifier: "PAP-3440" }, { type: "run_link", label: "Run", runId: "11111111-1111-4111-8111-111111111111" }] }] },
    });
  ...
  it("rejects arbitrary issue comment metadata", () => {
    const parsed = addIssueCommentSchema.safeParse({ body: "Hidden details", metadata: { version: 1, transcript: "raw log dump" } });
    expect(parsed.success).toBe(false);
  });

  it("requires mutationType when routed reviewer audit fields are present", () => {
    const parsed = addIssueCommentSchema.safeParse({ body: "Verdict", routedReviewerOf: "PAP-12", routingEvidenceLink: "/PAP/issues/PAP-77#comment-routing" });
    expect(parsed.success).toBe(false);
  });

  it("accepts both legacy and v1 routed reviewer mutation types", () => {
    const legacy = addIssueCommentSchema.parse({ body: "Legacy", mutationType: "routed_reviewer_comment", routedReviewerOf: "PAP-12", routingEvidenceLink: "/PAP/issues/PAP-77#comment-routing" });
    const v1 = addIssueCommentSchema.parse({ body: "V1", mutationType: "routed_reviewer_comment.v1", routedReviewerOf: "PAP-12", routingEvidenceLink: "/PAP/issues/PAP-77#comment-routing" });
