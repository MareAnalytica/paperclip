...
export function extractCodexRetryNotBefore(input: {
  ...
  // The captured clause (e.g. "11:31 PM (America/Chicago)") may carry a clock,
  // a date, or an inline parenthesised timezone — all handled by the shared,
  // provider-agnostic reset parser (ELI-864).
  return parseProviderResetTimestamp(usageLimitMatch[1] ?? "", now);
}
