...
describeEmbeddedPostgres("heartbeat stale queued-run invalidation", () => {
  ...
  it("runs execution-stage reviewer wakes even when reviewer is not the assignee", async () => {
    const { companyId, agentId } = await seedCompanyAndAgent();
    ...
    await db.insert(issues).values({
      id: issueId,
      companyId,
      title: "In-review task routed to reviewer",
      status: "in_review",
      priority: "medium",
      assigneeAgentId: agentId,
      executionState: {
  ...
  it("promotes execution-stage reviewer scheduled retries even when reviewer is not the assignee", async () => {
    const { companyId, agentId } = await seedCompanyAndAgent();
    const reviewerAgentId = randomUUID();
    await db.insert(agents).values({
      id: reviewerAgentId,
      companyId,
      name: "RetryReviewerAgent",
      role: "qa",
      status: "active",
      adapterType: "codex_local",
      adapterConfig: {},
    ...
    await db.insert(issues).values({
      id: issueId,
      companyId,
      title: "Retry in-review task routed to reviewer",
      status: "in_review",
      priority: "medium",
      assigneeAgentId: agentId,
      executionState: {
    ...
    const dueAt = new Date("2026-05-21T12:00:00.000Z");
    const { runId } = await seedQueuedRun({
      companyId,
      agentId: reviewerAgentId,
      issueId,
    ...
    await db
      ...
      .set({ status: "scheduled_retry", scheduledRetryAt: dueAt })
      .where(eq(heartbeatRuns.id, runId));
    ...
    expect(promotion).toMatchObject({ promoted: 1, runIds: [runId] });
