...
const issueListSelect = {
  ...
  blockerKind: issues.blockerKind,
    ...
    addComment: async (
      ...
      if (normalizedIdempotencyKey && actor.agentId) {
        const existing = await db
          ...
          .then((rows) => rows[0] ?? null);
        if (existing) {
          const [enrichedExisting] = await enrichCommentsWithDerivedAgentAttribution([existing]);
          const redactedExisting = redactIssueComment(enrichedExisting ?? existing, currentUserRedactionOptions.enabled);
      ...
      const [enrichedComment] = await enrichCommentsWithDerivedAgentAttribution([comment]);
      return redactIssueComment(enrichedComment ?? comment, currentUserRedactionOptions.enabled);
    },
