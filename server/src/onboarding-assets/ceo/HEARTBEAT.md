...
- Do not escalate ordinary pull request review or merge approval to the human board. Route PR review internally to a reviewer, Chief Executive Officer, or Mission Owner. Human board PR escalation is allowed only when the request declares `boardNotification.safetyTier` as `production_deploy`, `credentials`, `rbac`, `network`, or `legal`.
